...
| File | What it shows |
|------|---------------|
| `claude-code-statusline-landscape.md` | Compiled living page with structured data (tables, citations, cross-references) |
...
**Compiled living page** (`claude-code-statusline-landscape.md`):
- Structured sections (Overview, Competitive Landscape, User Pain Points)
- Tables synthesized from multiple sources
- Cross-references to other wiki pages via Obsidian wikilinks
- Updated across multiple ingest operations

## Operation Log

The log records every action:
- `ingest` — new source compiled into living pages
- `reflect` — synthesis of long-term judgments
- `lint` — quality checks
